...
# Connect to the SQLite database (adjust the database path if needed)
def get_connection():
    return sqlite3.connect("company_schedule.db")
...
def modify_employee_data(employee_id, name, role_id, start_date, end_date, nationality):
    try:
        conn = get_connection()
        cursor = conn.cursor()

        # Construct the SQL query to update the employee record
        query = """UPDATE Employees
                   SET Name = ?, RoleID = ?, StartDate = ?, EndDate = ?, Nationality = ?
                   WHERE EmployeeID = ?"""
        
        cursor.execute(query, (name, role_id, start_date, end_date, nationality, employee_id))
        conn.commit()
        conn.close()
...
# Additional functions for getting employees, etc., can go here if needed
