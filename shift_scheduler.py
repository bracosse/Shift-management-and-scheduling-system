...
def export_shifts_to_excel(year, month, output_file="Shifts schedule.xlsx"):
    conn = sqlite3.connect(DB_PATH)
    ...
    query = '''
        SELECT EmployeeID, EmployeeName, Role, Date, ShiftType
        FROM Shifts
        WHERE Date BETWEEN ? AND ?
    ...
    print(f"Shifts exported to Excel file: {output_file}")
###################################################

def assign_shifts_for_roles_gui(month_input):
    """
    Generates shifts for the specified month input (format: 'YYYY-MM') and returns a status message.
    """
    roles = get_employees_by_role()
    today = datetime.today()
    ...
    try:
        target_date = datetime.strptime(month_input, "%Y-%m")
        if target_date < today.replace(day=1) or target_date > (today.replace(day=1) + timedelta(days=62)):
            return "Invalid month range. Please pick current or next 2 months."
    except ValueError:
        return "Invalid format. Use YYYY-MM."

    year, month = target_date.year, target_date.month
    if check_if_shifts_generated_for_month(year, month):
        return f"Shifts for {calendar.month_name[month]} {year} are already generated."
    ...
    conn.commit()
    conn.close()
    return f"Shifts for {calendar.month_name[month]} {year} generated successfully."


def main():
    while True:
        month_input = input("Enter the month (YYYY-MM) to work with (or 'q' to quit): ").strip()
        ...
        print("\nChoose an option:")
        print("1. Generate Shifts for the Month")
        print("2. Export Existing Shifts to Excel")
        choice = input("Enter your choice (1 or 2): ").strip()

        if choice == '1':
            message = assign_shifts_for_roles_gui(month_input)
            print(message)
            if "generated successfully" in message:
                export_shifts_to_excel(year, month)
        elif choice == '2':
            if check_if_shifts_generated_for_month(year, month):
                export_shifts_to_excel(year, month)
            else:
                print(f"No shifts found for {calendar.month_name[month]} {year}. Please generate them first.")
        else:
